...
int64_t PartitionManager::update_centroid(int64_t partition_id, float* centroid_buffer) {
    ...
    if (delta_size == 0 || curr_partition->num_vectors_ == 0) {
        ...
        // the *current* centroid: downstream consumers (notably
        // estimate_uncertainty when uncertainty gating is enabled) feed
        // it straight in and would otherwise see a stale buffer left over
        // from a previous partition's update_centroid call.
